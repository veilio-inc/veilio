...
- Two-way anonymize / restore in-browser via `@veilio/shared`
...
> **Prerequisite:** `@veilio/shared` must be available on npm. Until then, see [docs/DEVELOPMENT.md](./docs/DEVELOPMENT.md) for local-link instructions.
...
The anonymizer engine (`@veilio/shared` on npm) is the same in both editions. The Cloud edition adds auth, encrypted server-side map storage, teams, and billing — none of which lives in this repo.

## Credits

Built on prior work by Igor Dlugosh in the (private) Veilio codebase. MIT licensed — use, modify, distribute freely.
